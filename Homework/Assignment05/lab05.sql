...
VALUES
	...
	(1, 7, 4, $3.50, 2018, GETDATE());
GO
--create table function with table name

create function CreateSupplyReport()
returns table
as
return 
	(select susa.Price, susa.QuantityPurchased, (select dbo.SupplyCost(susa.QuantityPurchased, susa.Price)) as SupplyCost, su.Name 
	from SuppliesSales susa
	inner join Supplies su on su.ID=susa.SupplyID);

Go

select * from dbo.createSupplyReport()
	order by SupplyCost desc;

--create scalar function for cost of inventory sales
if OBJECT_ID(N'dbo.SupplyCost', N'FN') IS NOT NULL
DROP FUNCTION SupplyCost;
GO

create function SupplyCost(@qty int, @price float)
returns dec(10,2)
as
begin
...
select dbo.SupplyCost(2, 112.48) as SupplyCost;